...
(async function(){
  ...
  async function loadImages(){
    ...
    function loadFromJs(){
      const man = (window.CHARGE_UP_MANIFEST && Array.isArray(window.CHARGE_UP_MANIFEST.laptopSkins))
        ? window.CHARGE_UP_MANIFEST.laptopSkins : [];
      return man;
    }

    const jsonList = await loadFromJson();
    const jsList = loadFromJs();
    const rawList = Array.from(new Set([...(jsonList||[]), ...(jsList||[])]));
  ...
  // Map subcategories to folder names under vandre
  const CATEGORY_MAP = {
    anime: 'anime and cartoone',
    cars: 'cars and bikes',
    marvel: 'marvel and movie',
    nature: 'nature',
    spiritual: 'spiritual',
    other: 'other'
  };
  ...
  function getCategoryFromHash(){
    ...
    return CATEGORY_MAP[h] ? h : '';
  }

  let allImages = await loadImages();
  ...
  function applyFilter(){
    ...
    if (!catKey){
      ...
      const folder = CATEGORY_MAP[catKey];
      const safe = folder.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const rx = new RegExp(`/vandre/${safe}/`, 'i');
      images = allImages.filter(p => rx.test(p.replace(/%20/g, ' ')));
    }
    totalItems = images.length;
    page = 1; // reset to first page when filter changes
    
    // Recalculate total pages and clamp page to valid range
    const totalPages = Math.max(1, Math.ceil(Math.max(0, totalItems) / pageSize));
    if (page > totalPages) page = totalPages;
    
    console.log(`Filter applied: ${catKey || 'all'}, ${totalItems} items, page ${page}/${totalPages}`);
    render();
  }
  ...
  async function render(){
    ...
    for(let i=start;i<end;i++){
      // Global numbering based on position in the full list
      const n = Math.max(1, (allImages.indexOf(images[i]) + 1) || (i + 1));
